...
const HabitCreateModalTabsV2 = ({ visible, onDismiss, onSuccess, onError, habit }: HabitCreateModalProps) => {
  const { user } = useAuth();
  ...
  const TabButton = ({ tab, label, icon }: { tab: typeof activeTab; label: string; icon: string }) => (
    ...
    >
      <View style={[styles.tabIconBox, activeTab === tab && styles.tabIconBoxActive]}>
  ...
  return (
    <Portal>
      <Modal visible={visible} onDismiss={onDismiss} contentContainerStyle={styles.modal}>
        ...
        {/* Prominent Tab Navigation with Icons */}
        <View style={styles.tabs}>
          <TabButton tab="basic" label="Basic Info" icon="ℹ️" />
          <TabButton tab="schedule" label="Schedule" icon="📅" />
          <TabButton tab="custom" label="Styling" icon="🎨" />
        </View>
...
export default HabitCreateModalTabsV2;
